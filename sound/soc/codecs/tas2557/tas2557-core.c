									...
									int nPrevConfig, int nNewConfig, bool bPowerOn)
{
	...
	if (pPrevConfiguration) {
		if ((pPrevConfiguration->mnPLL == pNewConfiguration->mnPLL) && (pPrevConfiguration->mnDevices == pNewConfiguration->mnDevices)) {
...
int tas2557_set_sampling_rate(struct tas2557_priv *pTAS2557, unsigned int nSamplingRate)
{
		 ...
		 nConfiguration++)
	{
		...
		if ((pConfiguration->mnSamplingRate == nSamplingRate) && (pConfiguration->mnProgram == pTAS2557->mnCurrentProgram)) {
									   ...
									   struct TFirmware *pFirmware, unsigned char *pData)
{
	...
	for (nConfiguration = 0; nConfiguration < pFirmware->mnConfigurations;
		...
		if ((pFirmware->mnDriverVersion >= PPC_DRIVER_CONFDEV) || ((pFirmware->mnDriverVersion >= PPC_DRIVER_CFGDEV_NONCRC) && (pFirmware->mnDriverVersion < PPC_DRIVER_CRCCHK))) {
							   ...
							   unsigned char nBook, unsigned char nPage, unsigned char nReg, unsigned char nValue)
{
	...
	if ((nBook == TAS2557_BOOK_ID(TAS2557_SA_COEFF_SWAP_REG)) && (nPage == TAS2557_PAGE_ID(TAS2557_SA_COEFF_SWAP_REG)) && (nReg >= TAS2557_PAGE_REG(TAS2557_SA_COEFF_SWAP_REG)) && (nReg <= (TAS2557_PAGE_REG(TAS2557_SA_COEFF_SWAP_REG) + 4))) {
							  ...
							  unsigned char nBook, unsigned char nPage, unsigned char nReg, unsigned int len)
{
	...
	if ((nBook == TAS2557_BOOK_ID(TAS2557_SA_COEFF_SWAP_REG)) && (nPage == TAS2557_PAGE_ID(TAS2557_SA_COEFF_SWAP_REG)) && (nReg == TAS2557_PAGE_REG(TAS2557_SA_COEFF_SWAP_REG)) && (len == 4)) {
		/* DSP swap command, pass */
		nResult = 0;
		goto end;
	}
	...
	if (nResult == 1) {
		if (len == 1) {
			...
			for (i = 0; i < TCRCData.mnLen; i++) {
				if ((nBook == TAS2557_BOOK_ID(TAS2557_SA_COEFF_SWAP_REG)) && (nPage == TAS2557_PAGE_ID(TAS2557_SA_COEFF_SWAP_REG)) && ((i + TCRCData.mnOffset) >= TAS2557_PAGE_REG(TAS2557_SA_COEFF_SWAP_REG)) && ((i + TCRCData.mnOffset) <= (TAS2557_PAGE_REG(TAS2557_SA_COEFF_SWAP_REG) + 4))) {
...
static int tas2557_load_block(struct tas2557_priv *pTAS2557, struct TBlock *pBlock)
{
	...
	} else if ((pBlock->mnType == TAS2557_BLOCK_PGM_DEV_A) || (pBlock->mnType == TAS2557_BLOCK_CFG_COEFF_DEV_A) || (pBlock->mnType == TAS2557_BLOCK_CFG_PRE_DEV_A)) {
		chl = channel_left;
	} else if ((pBlock->mnType == TAS2557_BLOCK_PGM_DEV_B) || (pBlock->mnType == TAS2557_BLOCK_CFG_COEFF_DEV_B) || (pBlock->mnType == TAS2557_BLOCK_CFG_PRE_DEV_B)) {
		chl = channel_right;
									  ...
									  unsigned int nConfiguration, bool bLoadSame)
{
	...
	if ((!pTAS2557->mbLoadConfigurationPrePowerUp) && (nConfiguration == pTAS2557->mnCurrentConfiguration) && (!bLoadSame)) {
	...
	for (i = 0; i < pBlock->mnCommands;) {
		...
		if ((nOffset < 0x7f) || (nOffset == 0x81))
			...
			if ((len + nOffset) >= (TAS2557_PAGE_REG(nReg) + 4)) {
				...
				nCoefficient = ((int)pCommands[n] << 24) | ((int)pCommands[n + 1] << 16) | ((int)pCommands[n + 2] << 8) | (int)pCommands[n + 3];
				bFound = true;
				break;
			}
		} else {
