...
static ssize_t tas2557_file_read(struct file *file, char *buf, size_t count, loff_t *ppos)
{
	...
	case TIAUDIO_CMD_REG_READ: {
		...
		if (count == 1) {
			ret = pTAS2557->read(pTAS2557, pTAS2557->mnCurrentChannel, pTAS2557->mnCurrentReg, &nValue);
			if (ret < 0)
				break;
			...
			if (p_kBuf != NULL) {
				ret = pTAS2557->bulk_read(pTAS2557, pTAS2557->mnCurrentChannel, pTAS2557->mnCurrentReg, p_kBuf, count);
				if (ret < 0) {
	...
	case TIAUDIO_CMD_PROGRAM: {
		if ((pTAS2557->mpFirmware->mnConfigurations > 0) && (pTAS2557->mpFirmware->mnPrograms > 0)) {
			if (g_logEnable)
...
static ssize_t tas2557_file_write(struct file *file, const char *buf, size_t count, loff_t *ppos)
{
	...
	case TIAUDIO_CMD_REG_WITE:
		if (count > 6) {
			chl = p_kBuf[1];
			reg = ((unsigned int)p_kBuf[2] << 24) + ((unsigned int)p_kBuf[3] << 16) + ((unsigned int)p_kBuf[4] << 8) + (unsigned int)p_kBuf[5];
			len = count - 6;
			if (len == 1) {
	...
	case TIAUDIO_CMD_REG_READ:
		if (count == 6) {
			...
			pTAS2557->mnCurrentReg = ((unsigned int)p_kBuf[2] << 24) + ((unsigned int)p_kBuf[3] << 16) + ((unsigned int)p_kBuf[4] << 8) + (unsigned int)p_kBuf[5];
			if (g_logEnable) {
				dev_info(pTAS2557->dev,
